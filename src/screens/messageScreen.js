...
import { SafeAreaView, ActivityIndicator, KeyboardAvoidingView, Platform, Keyboard, TouchableWithoutFeedback, View, Text, Image, TouchableOpacity, TextInput, StyleSheet, Dimensions, FlatList } from 'react-native';
...
import { messagesByConversation, getUser, getConversation } from '../graphql/queries';
...
export default function MessageUserScreen({ navigation, route }) {
    ...
    let { conversationID, userID, callback } = route.params;
    ...
    const fetchConversation = async() => {
        ...
        let cd = await API.graphql(graphqlOperation(getConversation, { id: conversationID }));
        let co = cd.data.getConversation;
        if (co.senderID !== user.id) {
            //user is recipient
            setIsSender(false);
            let cinfo = {
                id: conversationID,
            ...
            let updatedConversation = await API.graphql(graphqlOperation(updateConversation, { input: cinfo }));
        } else {
            let cinfo = {
                id: conversationID,
        ...
        let md = await API.graphql(graphqlOperation(messagesByConversation, { conversationID: conversationID, sortDirection: 'ASC' }));
    ...
    const handleSubmit = async() => {
        ...
        setMessages([...msgarr]);
        Analytics.record('sendMessage', { userID: currentUserID, content: input});
    ...
    const renderMessage = ({ item }) => {
        return (
            <Message message={item} previousDates={previousDates} setPreviousDates={setPreviousDates} recipientID={userID} />
        )
    }

    if (loading) {
        return <SafeAreaView style={GlobalStyles.droidSafeArea}>
            <BasicHeader name='Messages' />
    ...
    return (
        <SafeAreaView style={GlobalStyles.droidSafeArea}>
            <KeyboardAvoidingView style={{flex: 1}} behavior={Platform.OS === "ios" ? "padding" : "null"} enabled>
                        <BasicHeader name={"Messages with @" + friend.username} />
                        <View style={{flex: 2.5}}>
